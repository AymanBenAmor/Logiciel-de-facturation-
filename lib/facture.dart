...
class _FacturePageState extends State<FacturePage> {
  ...
  final filePath = '${directory.path}/AchrafApp/BL/${DateTime.now().year.toString()}/${DateTime.now().month.toString()}/$clientName.csv';
  ...
  if (!await file.exists()) {
    ...
    SnackBar(content: Text('Ce client n\'admet pas de facture pour cet mois.')),
  );
    return ; // File doesn't exist, so the code client can't exist either
  }
  ...
  for(var row in rows){
    ...
    setState(() {
      foundReferences.add(row[0].toString());
      Designation.add(row[1].toString());
      
      quantity.add(row[2].toString());
      PU.add(row[3].toString());
      Remise.add(row[4].toString());
      TVA_list.add(row[5].toString()+'%');
      montant.add(row[6].toString());
    });
  }
...
pw.Column generateTextColumn_string( List<String> list) {
  ...
  for (int i = 0; i < list.length; i++) {
    if(list[i].startsWith('BL N')){
      texts.add(pw.Text(list[i], style: pw.TextStyle(fontSize: 8,fontWeight:pw.FontWeight.bold )));
      ...
      texts.add(pw.Text(
        ...
        style: pw.TextStyle(
          fontSize: 8,
          color: PdfColors.white, // Correct usage of white color
        ),
      ));
      texts.add(pw.SizedBox(height: 6));
...
double calculateTotalHT() {
  ...
  for (int i = 0; i < PU.length; i++) {
    double puValue = double.tryParse(PU[i]) ?? 0.0;
    double quantityValue = double.tryParse(quantity[i]) ?? 0.0; // Parse PU as a double or default to 0.0
         // Use quantity[i] or default to 0.0
...
double calculateRemise() {
  ...
  for (int i = 0; i < PU.length; i++) {
    ...
    double remiseValue = double.tryParse(Remise[i]) ?? 0.0;
 

    result += quantityValue * puValue * remiseValue / 100; // Add the product to the result
  }
...
String getRemise(String remiseValue){
  if(remiseValue == ''){
    return remiseValue;
  }
  return (remiseValue + '%');
...
Future<String> getFilePath_numero() async {
    final directory = await getApplicationDocumentsDirectory();
    final pdfDirectory = Directory('${directory.path}/AchrafApp/factures/${DateTime.now().year.toString()}');
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.Row(
            ...
            children: [
              pw.Text(
                "F $numero_facture/${DateTime.now().year.toString().substring(2)}", // Extract the last 2 digits of the current year
                textAlign: pw.TextAlign.center,
              ),
              pw.Text(
...
pw.TableRow(
  children: [
    ...
    pw.Container(
      ...
      alignment: pw.Alignment.center,
      padding: pw.EdgeInsets.all(5),
      child: generateTextColumn_string(quantity),
    ...
    pw.Container(
      ...
      padding: pw.EdgeInsets.all(5),
      alignment: pw.Alignment.center,
      child: generateTextColumn_remise(Remise),
    ...
    pw.Container(
      ...
      alignment: pw.Alignment.center,
      padding: pw.EdgeInsets.all(5),
      child: generateTextColumn_string(TVA_list),
    ),
    pw.Container(
      height: 310,
      alignment: pw.Alignment.center,
      padding: pw.EdgeInsets.all(5),
      child: generateTextColumn_string(montant),
...
pw.Row(
  ...
  final directory = await getApplicationDocumentsDirectory();
  final pdfDirectory = Directory('${directory.path}/AchrafApp/factures/${DateTime.now().year.toString()}/${DateTime.now().month.toString()}');
  ...
  final filePath = '${pdfDirectory.path}/facture N° $numero_facture.pdf';
...
try {
  ...
  await file.writeAsBytes(pdfBytes); // Save the PDF
   ...
   ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text('Facture saved in: $filePath')),
  );
...
deleteCsvFile('${directory.path}/AchrafApp/BL/${DateTime.now().year.toString()}/${DateTime.now().month.toString()}/$clientName.csv');
  ...
  child: ListView.builder(
    ...
    itemBuilder: (context, index) {
      return Padding(
        ...
        child: Text(
          foundReferences[index], // Display the designation at the current index
    ...
    itemBuilder: (context, index) {
      return Padding(
        ...
        child: Text(
          ...
          style: TextStyle(
            fontSize: 9,
            fontWeight: Designation[index].startsWith('BL N')
                ? FontWeight.bold
                : FontWeight.normal, // Make text bold if it starts with 'BL N'
          ),
        ),
      );
    },
...
Container(
  ...
  padding: EdgeInsets.all(5),
  child: ListView.builder(
    itemCount: quantity.length, // Use the length of the list
    itemBuilder: (context, index) {
      return Padding(
        padding: const EdgeInsets.only(bottom: 5.0),
        child: Text(
          quantity[index].toString(), // Display the designation at the current index
          style: TextStyle(fontSize: 9),
        ),
      );
    },
...
// Remise
  ...
  padding: EdgeInsets.all(5),
  child: ListView.builder(
    itemCount: Remise.length, // Use the length of the list
    itemBuilder: (context, index) {
      return Padding(
        padding: const EdgeInsets.only(bottom: 5.0),
        child: Text(
          getRemise(Remise[index]), // Display the designation at the current index
          style: TextStyle(fontSize: 9),
        ),
      );
    },
  ...
  child: ListView.builder(
    ...
    itemBuilder: (context, index) {
      return Padding(
        padding: const EdgeInsets.only(bottom: 5.0),
        child: Text(
          getTVA(TVA_list[index]) , // Display the designation at the current index
          style: TextStyle(fontSize: 9),
        ),
      );
    },
  ...
  child: ListView.builder(
    ...
    itemBuilder: (context, index) {
      return Padding(
        padding: const EdgeInsets.only(bottom: 5.0),
        child: Text(
          montant[index].toString(), // Display the designation at the current index
          style: TextStyle(fontSize: 9),
        ),
      );
    },
